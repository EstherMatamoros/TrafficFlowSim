...
speeds = {'person1':0.5, 'person2':0.4, 'person3':0.4, 'person4':0.6}  # average speeds of pedestrians
...
pedestrians = {'right': {0:[], 1:[], 2:[], 'crossed':0}, 'down': {0:[], 1:[], 2:[], 'crossed':0}, 'left': {0:[], 1:[], 2:[], 'crossed':0}, 'up': {0:[], 1:[], 2:[], 'crossed':0}}
...
stoppingGap = 7    # stopping gap
movingGap = 7   # moving gap
...
class Pedestrian(pygame.sprite.Sprite):
    def __init__(self, lane, pedestrianClass, direction_number, direction):
        ...
        if(len(pedestrians[direction][lane])>1 and pedestrians[direction][lane][self.indea-1].crossed==0):    # if more than 1 pedestrian in the lane of pedestrian before it has crossed stop line
            if(direction=='right'):
                self.stop = pedestrians[direction][lane][self.indea-1].stop - pedestrians[direction][lane][self.indea-1].image.get_rect().width - stoppingGap         # setting stop coordinate as: stop coordinate of next pedestrian - width of next pedestrian - gap
            elif(direction=='left'):
                self.stop = pedestrians[direction][lane][self.indea-1].stop + pedestrians[direction][lane][self.indea-1].image.get_rect().width + stoppingGap
            elif(direction=='down'):
                self.stop = pedestrians[direction][lane][self.indea-1].stop - pedestrians[direction][lane][self.indea-1].image.get_rect().height - stoppingGap
            elif(direction=='up'):
                self.stop = pedestrians[direction][lane][self.indea-1].stop + pedestrians[direction][lane][self.indea-1].image.get_rect().height + stoppingGap
        else:
    ...
    def move(self):
        if(self.direction=='right'):
            if(self.crossed==0 and self.a+self.image.get_rect().width>stopLinesP[self.direction]):   # if the image has crossed stop line now
                self.crossed = 1
            if((self.a+self.image.get_rect().width<=self.stop or self.crossed == 1 or (currentPGreen==0)) and (self.indea==0 or self.a+self.image.get_rect().width<(pedestrians[self.direction][self.lane][self.indea-1].a - movingGap))):                
            ...
            if(self.crossed==0 and self.b+self.image.get_rect().height>stopLinesP[self.direction]):
                self.crossed = 1
            if((self.b+self.image.get_rect().height<=self.stop or self.crossed == 1 or (currentPGreen==1)) and (self.indea==0 or self.b+self.image.get_rect().height<(pedestrians[self.direction][self.lane][self.indea-1].b - movingGap))):                
                self.b += self.speed
        elif(self.direction=='left'):
            if(self.crossed==0 and self.a<stopLinesP[self.direction]):
                self.crossed = 1
            if((self.a>=self.stop or self.crossed == 1 or (currentPGreen==2 )) and (self.indea==0 or self.a>(pedestrians[self.direction][self.lane][self.indea-1].a + pedestrians[self.direction][self.lane][self.indea-1].image.get_rect().width + movingGap))):                
                self.a -= self.speed   
        elif(self.direction=='up'):
            if(self.crossed==0 and self.b<stopLinesP[self.direction]):
                self.crossed = 1
            if((self.b>=self.stop or self.crossed == 1 or (currentPGreen==3)) and (self.indea==0 or self.b>(pedestrians[self.direction][self.lane][self.indea-1].b + pedestrians[self.direction][self.lane][self.indea-1].image.get_rect().height + movingGap))):                
                self.b -= self.speed
...
def repeat():
    ...
    signals[nextPGreen].red = signals[currentPGreen].green    # set the red time of next to next signal as (yellow time + green time) of next signal
    repeat()  
...
class Main:
    ...
    thread2 = threading.Thread(name="generatepedestrians",target=generatepedestrians, args=())    # Generating pedestrians
    thread2.daemon = True
    thread2.start()
